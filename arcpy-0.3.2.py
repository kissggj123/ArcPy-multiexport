...
1.修复了可能遇到中文崩溃问题
2.添加了一个导出进度的指示器
[首次配置：python arcpy-0.3.2.py --configure]
[直接运行：python arcpy-0.3.2.py]运用多进程技术导出图片
    集成Tkinter
    使用了多进程技术
"""
# ---------------------------------------------------------------------------
...
def export_jpeg(path_slice_set, res, output_folder):
    ...
    total_jobs = len(path_slice_set)
    
    for i, one_path in enumerate(path_slice_set):
        ...
        # 更新导出进度
        progress = int((float(i + 1) / total_jobs) * 100)
        progress_dict[i] = progress
...
def main_function(args):
    ...
    with Manager() as manager:
        ...
        for set_li in sets_lists:
            ...
            # 监听导出进度并打印百分比
            while len(progress_dict) < len(set_li):
                time.sleep(0.1)
                progress = sum(progress_dict.values()) / len(progress_dict)
                print("导出进度: {}%".format(progress))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--changelog', action='store_true', help='显示更新日志')

    args = parser.parse_args()

if args.changelog:
    with open('changelog.txt', 'r') as changelog_file:
        changelog = changelog_file.read().rstrip()
        print("更新日志:\n")
        print(changelog)
